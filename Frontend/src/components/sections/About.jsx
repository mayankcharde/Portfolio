      ...
      <div className="container mx-auto px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center mb-24">
          ...
          >
            ...
            <p className="text-muted text-base leading-relaxed mb-10">
              {personalInfo.bio} I specialize in bridging the gap between design
              and development, ensuring every project I touch is both
              functionally robust and aesthetically superior.
            </p>
              ...
              {[
                {
                  ...
                  value: parseInt(stats.projects),
                  suffix: "+",
                },
                {
                  label: "GitHub Commits",
                  value: parseInt(stats.commits),
                  suffix: "+",
                },
                {
                  label: "Pull Requests",
                  value: parseInt(stats.prs),
                  suffix: "+",
                },
                {
                  label: "Technologies",
                  value: parseInt(stats.technologies),
                  suffix: "+",
                },
              ].map((stat, i) => (
                <div
                  key={i}
                  className="p-5 rounded-xl bg-white/[0.02] border border-white/[0.04] hover:border-white/[0.08] transition-colors group/stat"
                >
                  <div className="text-2xl font-heading font-black text-accent mb-1 group-hover/stat:gradient-text transition-all">
                    <AnimatedCounter to={stat.value} suffix={stat.suffix} />
                  </div>
                  <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-accent/30">
                    {stat.label}
                  </p>
                </div>
              ))}
            ...
            <div className="flex flex-wrap gap-3">
              <a
                ...
                className="px-7 py-3 bg-secondary/90 text-white font-bold text-xs uppercase tracking-[0.12em] rounded-xl hover:shadow-glow-violet transition-all"
              >
                Download Resume
              </a>
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="px-7 py-3 border border-white/10 text-accent/70 font-bold text-xs uppercase tracking-[0.12em] rounded-xl hover:bg-white/[0.03] hover:border-white/20 transition-all"
              >
                View GitHub
              </a>
            </div>
        ...
        <div className="flex flex-wrap justify-center gap-3">
          {funFacts.map((fact, i) => (
            ...
            >
              <span
                className={`text-sm ${fact.color} group-hover:scale-110 transition-transform`}
              >
                {fact.icon}
              </span>
              <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-accent/50 group-hover:text-accent/70 transition-colors">
                {fact.text}
              </span>
            </motion.div>
          ))}
